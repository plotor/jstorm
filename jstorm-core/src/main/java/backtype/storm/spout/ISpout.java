 ...
 * For every tuple emitted by a spout, Storm will track the (potentially very large) DAG（有向无环图） of tuples generated
 * based on a tuple emitted by the spout.
 * When Storm detects that every tuple in that DAG has been successfully processed, it will send an ack message to the Spout.
 *
 * <p>
 * If a tuple fails to be fully processed within the configured timeout for the topology (see {@link backtype.storm.Config}),
 * Storm will send a fail message to the spout for the message.
 * </p>
 *
 * <p>
 * When a Spout emits a tuple, it can tag the tuple with a message id. The message id can be any type.
 ...
 * If the spout leaves out the message id, or sets it to null, then Storm will not track the message and the spout will not receive any ack or fail callbacks for the message.
 * </p>
 *
 * <p>
 * Storm executes ack, fail, and nextTuple all on the same thread.
 * This means that an implementor of an ISpout does not need to worry about concurrency issues between those methods.
 * However, it also means that an implementor must ensure that nextTuple is non-blocking:
...
public interface ISpout extends Serializable {
     ...
     * There is no guarentee that close will be called, because the supervisor kill -9's worker processes on the cluster.
     *
     * <p>
    ...
    /**
     * Called when a spout has been activated out of a deactivated mode. <code>nextTuple</code> will be called on this spout soon.
     * A spout can become activated after having been deactivated when the topology is manipulated using the `storm` client.
     */
    void activate();

    /**
     * Called when a spout has been deactivated. <code>nextTuple</code> will not be called while a spout is deactivated.
     ...
     *
     * 顾名思义，就是获取下一条消息。
     * 执行时，可以理解成 JStorm 框架会不停地调这个接口，以从数据源拉取数据并往 bolt 发送数据。
     */
    void nextTuple();
     ...
     * Typically, an implementation of this method will put that message back on the queue to be replayed at a later time.
     */
