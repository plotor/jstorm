 ...
 */
public class SlidingIngestionTimeWindows<T> extends WindowAssigner<T> {
    ...
    private SlidingIngestionTimeWindows(long size, long slide) {
        this.size = size;
        this.slide = slide;
    }
    ...
    public Collection<TimeWindow> assignWindows(T element, long timestamp) {
        timestamp = ((TupleExt) element).getCreationTimeStamp();
     ...
     */
    public static <T> SlidingIngestionTimeWindows<T> of(long size, long slide) {
        return new SlidingIngestionTimeWindows<>(size, slide);
    }
}
